(ns sap-plm.main
  "Kotodama WASM entrypoint for the SAP PLM clean-room actor (L4) — Clojure port.
  ...
  facts (`sap_plm.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sap_plm")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"sap_plm.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sap_plm-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sap_plm-compat/ready)
